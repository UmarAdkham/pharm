...
export default function useWholesaleCompanyData(apiPath, isReport) {
      ...
      try {
        ...
        const rows = wscs.map((wsc) => {
          const row = {
            ...
            onClick: () =>
              isReport
                ? navigate(`/ws/report/${wsc.id}`, { state: wsc.name })
                : navigate(`/ws/company-info/${wsc.id}`),
          };
                ...
                : {
                    actions: (
                      <IconButton
                        onClick={(e) => {
                          e.stopPropagation();
                          handleEditClick(updatedCompany);
                        }}
                        sx={{
                          "&:hover": {
                            backgroundColor: "#e0f2f1",
